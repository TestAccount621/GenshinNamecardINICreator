...
namespace GenshinNamecardINICreator.CreationClasses
{
    public class NamecardSingleINI
    {
        ...
        public NamecardSingleINI() { }

        public async Task CreateSingleINI(DirectoryInfo name, int swapcard, NamecardData namecard)
        {
            try
            {
                ...
                fileNames.Sort(comparer: new NumericStringComparer());
                foreach (var fi in fileNames)
                {
                    if (Int32.TryParse(fi.Last().ToString(), out int test))
                    {
                        sortedFiles.Add(fi);
                    }
                    else
                ...
                if (sortedFiles.Count > 0)
                {
                    ...
                    {
                        await CreateNameCardSingleImage_INI(swapcard, namecard);
                    }
                    else if (sortedFiles.Count > 1)
                    {
                        await CreateNameCardGIF_INI(swapcard, namecard);
                    }
                }
            }
            catch (Exception)
        ...
        private async Task CreateNameCardGIF_INI(int swapcard, NamecardData namecard)
        {
            await Task.Run(() =>
            {
                try
        ...
        private async Task CreateNameCardSingleImage_INI(int swapcard, NamecardData namecard)
        {
            await Task.Run(() =>
            {
                try
